...
Reversi::Reversi():
            gameCells(checkBoardSizes(2000),
                                vector<Cell> (checkBoardSizes(2000)))
{
    setboardHeight(2000);
    setboardWidth(2000);
    setComputerCanMove(1);
...
//
//
//Operator ++ (Prefix version) will advance the game by one step for computer. 
...
//Operator ++ (Postfix version) will advance the game by one step for computer. 
//It will return the expected results.
...
Cell& Reversi::operator [](string index)
{   
    int cellPositionX=0; // rakamlari tutan degisken
    int digitNumber=1; // basamagin degerini (birler, onlar vb.) tutar.
    int cellPositionYNumber; // cell Position Y Stringinin sayi karsiligi
    int digit;
    string cellPositionY;
    string temp;
    
    int error=0; // Girilen degerin hatali olup olmadigi
    int checkLetterPart=0; //Harf kısmına gecildigini temsil eder
                           //Eger 1 ise stringin harf kismi okunuyodur.
    int checkNumberPart=1; //Sayı kısmına gecildigini ifade eder
                           ////Eger 1 ise stringin sayi kismi okunuyodur.
    int firstTime=1;
    
    //once sayi kismi okunur.
    for (string::iterator i=index.end()-1; i!=index.begin()-1; --i)
    {   
        if(checkLetterPart==1 || *i=='A' || *i=='B' || *i=='C' || *i=='D'
            || *i=='E' || *i=='F' || *i=='G' || *i=='H' || *i=='I' || *i=='J')
        {
            checkLetterPart=1;
            temp+=*i;
            checkNumberPart=0;
        ...
        else if(checkNumberPart)
        {
            int tempI = (*i) - '0';    
            firstTime=0;
            cellPositionX+=digitNumber*tempI;
            digitNumber = digitNumber*10; // basamak degeri arttirilir
        }
        
        if(checkLetterPart==1 && checkNumberPart==1)
        {
            garbage.setX(-1000);
            garbage.setY(-1000);
            garbage.setSembol(0);
    ...
    for (string::iterator i=temp.end()-1; i!=temp.begin()-1; --i)
        cellPositionY+=*i;
    
    
    cellPositionYNumber=stringTranslateNumber(cellPositionY);
    
    if(cellPositionX>0 && cellPositionX<=getBoardWidth() &&
       cellPositionYNumber>0 && cellPositionYNumber<=getBoardHeight())
    {
       return gameCells[cellPositionX-1][cellPositionYNumber-1];
    }
...
Cell& Reversi::operator ()(string index)
{
    Reversi temp(getBoardWidth(), getBoardHeight());
    garbage=temp[index];
    return garbage;
}

//that takes another a Cell object and plays the game for the user.
Cell&  Reversi::operator +=(const Cell& move)
{
    int cellPositionYNumber;
    int tempColumbNumber=move.getY();
    int positionX=move.getX();
    int columbNumberDigits=0;
    int digit;
    int number=move.getY();
    char letter;
    vector <char> tempdigitToLetter;
    string positionY;
    
    //Eger Player Hamle yapabiliyorsa
    if(getPlayerCanMove())  
    {        
        //(columb) sayisinin kac basamak oldugu bulunur.
        while (tempColumbNumber) {
            tempColumbNumber /= 10;
            ++columbNumberDigits;
        ...
        while(number)
        {
            ...
            letter=digitTranslateLetter(digit); // basamagin harf karsiligi
                                                // bulunur.
            tempdigitToLetter.push_back(letter); // vektore bulunan harf eklenir.
        }   
        
        //Sayinin harf karsiligi yazilir
        for(int j=columbNumberDigits-1; j>=0 ; --j)
            positionY+=tempdigitToLetter[j];
        
        //Guncel board ekrana cizilir.
        displayBoard();
                
        //A function that plays the player move
        playPlayer(positionX, positionY);              
        ...
        cout << endl << "The player does not have a legal move" << endl;
    
     if(move.getX()>0 && move.getX()<=getBoardWidth() &&
       move.getY()>0 && move.getY()<=getBoardHeight() && getPlayerCanMove()==1)
    {
         
       cout << "The player's move is " << move.getX() << positionY << endl;  
         
       return gameCells[move.getX()][move.getY()];
    }
